...
class Scan(Operator):
    ...
    def get_next(self):
        ...
        for i in range(self.batchsize):
            ...
            if line:
                result.append(ATuple([int(i) for i in line.split()]))
            else:
                self.EOF=True
                return result
        return result
        ...
        for i in range(len(cur_left_input)):
            for j in range(len(cur_right_input)):
                if cur_left_input[i].tuple[self.left_attr]==cur_right_input[j].tuple[self.right_attr]:
                    result.append(ATuple(cur_left_input[i].tuple+cur_right_input[j].tuple))
        return result
        pass
