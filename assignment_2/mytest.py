...
#******************************* Tests for 3 tasks of Assignment 1***********************************************
...
#********************************Tests for 4 tasks of Assignment 2*************************************************** 
...
def test_task5():  
    ...
    def cmp(tuple):
        return tuple.tuple[1]
    F=Scan("tiny_data_friends.txt",track_prov=True)         
    R=Scan("tiny_data_ratings.txt",track_prov=True)        
    ...
    assert result.get_next()[0].lineage()==[ATuple([0,1]),ATuple([0,2]),ATuple([1,0,4]),ATuple([2,0,5])]
...
def test_task6():
    ...
    assert grouped.get_next()[0].where(0)==[('tiny_data_ratings.txt',5,(1,1,1),1)\
                                            ,('tiny_data_ratings.txt',8,(2,1,0),0)]

def test_task7():
    F=Scan("tiny_data_friends.txt",propagate_prov=True)        
    R=Scan("tiny_data_ratings.txt",propagate_prov=True)   
    def funcF(tuple):
        if int(tuple.tuple[0])==0:
            return True
        else :
    ...
    assert result.get_next()[0].how()=="AVG('(f1*r4@4)', '(f2*r7@5)')"       
...
def test_task8():
    ...
    def cmp(tuple):
        return tuple.tuple[1]
    F=Scan("tiny_data_friends.txt",track_prov=True,propagate_prov=True)     
    R=Scan("tiny_data_ratings.txt",track_prov=True,propagate_prov=True)        
    ...
    assert result.get_next()[0].responsible_inputs()==[((0, 1), 1), ((1, 0, 4), 1), ((0, 2), 1), ((2, 0, 5), 1)]
